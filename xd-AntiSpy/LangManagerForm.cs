    ...
    public partial class LangManagerForm : Form
    {
        ...
        private async void LoadLanguages()
        {
            ...
            try
            {
                ...
                // Bind the list of languages to our ComboBox
        ...
        private async void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            string downloadUrl = $"https://github.com/builtbybel/xd-AntiSpy/raw/main/languages/{selectedLanguageCode}/LocalizationLibrary.resources.dll";
            string appDirectory = AppDomain.CurrentDomain.BaseDirectory;
            string languageDirectory = Path.Combine(appDirectory, selectedLanguageCode);
            string destinationFilePath = Path.Combine(languageDirectory, "LocalizationLibrary.resources.dll");
        ...
        private void SaveLanguageToJson(string cultureCode)
        {
            string filePath = "lang.json";

            var jsonObj = new { language = cultureCode };

            try
            {
                string jsonContent = JsonConvert.SerializeObject(jsonObj, Formatting.Indented);
                File.WriteAllText(filePath, jsonContent);
                MessageBox.Show($"Language {cultureCode} saved successfully. The application will restart to apply changes.");
                Application.Restart();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error saving language file: {ex.Message}");
            }
        }

        private void comboBoxLanguages_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (isInitialized && comboBoxLanguages.SelectedItem != null)
            {
                string selectedLanguageItem = comboBoxLanguages.SelectedItem.ToString();
                string selectedLanguageCode = selectedLanguageItem.Split(' ')[0];

                SaveLanguageToJson(selectedLanguageCode);
            }
        }
    }
}